  ...
  return (
    <div className='overflow-auto'>
      <table className='min-w-full bg-white dark:bg-gray-800 text-sm'>
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item._id} className='hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700'>
              <td className='py-2 px-4 border-b'>
                <div className='flex'>
                  <Image src={item.template.icon} alt='icon' width={20} height={20} />
                  <div className='ml2'>{item.template.name}</div>
                </div>
              </td>
              <td className='py-2 px-4 line-clamp-3'>{item.query}</td>
              <td className='py-2 px-4 border-b'>{new Date(item.createdAt).toLocaleDateString()}</td>
              <td className='py-2 px-4 border-b'>{wordCount(item.query)}</td>
              <td className='py-2 px-4 border-b'>
